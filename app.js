...
function displayWeather(data){
  ...
  document.getElementById("currentWeather").innerHTML=`<h2>${l.name}, ${l.country}</h2><img src="https:${c.condition.icon}" alt="${c.condition.text}"><h1>${c.temp_c}°C</h1><p>${c.condition.text}</p>`;
  let html="";
  data.forecast.forecastday.forEach(d=>{
    html+=`<div class="forecast-day"><p>${d.date}</p><img src="https:${d.day.condition.icon}" alt="${d.day.condition.text}"><p>${d.day.maxtemp_c}° / ${d.day.mintemp_c}°</p></div>`;
  });
